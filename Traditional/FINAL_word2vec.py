...
# # 2. Split data (BEFORE any embedding)
...
def train_word2vec(sentences, vector_size=100, window=5, min_count=2, workers=4, epochs=10):
...
def get_doc_embedding(model, text):
...
# Train Word2Vec on ALL available text data (for better word coverage)
word2vec_model = train_word2vec(texts.tolist())

# Generate embeddings for train and test sets
X_train_embedded = np.array([get_doc_embedding(word2vec_model, text) for text in X_train])
X_test_embedded = np.array([get_doc_embedding(word2vec_model, text) for text in X_test])
...
for name, model in models.items():
    ...
    y_proba = model.predict_proba(X_test_embedded)[:, 1] if hasattr(model, "predict_proba") else [0]*len(y_test)
...
results_df.to_excel("FINAL_word2vec_Embedding.xlsx")
