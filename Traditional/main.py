...
evaluations = ['accuracy', 'precision', 'recall', 'f1', 'roc_auc']
...
bert_df = pd.read_excel('bert_metrics_full.xlsx', index_col='Model')
count_df = pd.read_excel('count_metrics_full.xlsx', index_col='Model')
gemini_df = pd.read_excel('gemini_metrics_full.xlsx', index_col='Model')
tfidf_df = pd.read_excel('tfidf_metrics_full.xlsx', index_col='Model')
word2vec_df = pd.read_excel('word2vec_metrics_full.xlsx', index_col='Model')
...
# fig, ax = plt.subplots(figsize=(12, 6))
# im = ax.imshow(accuracy, cmap='YlGnBu', vmin=0.5, vmax=1)
# ax.set_xticks(range(len(models)), labels=models,
#               rotation=45, ha="right", rotation_mode="anchor")
...
bert_recall = bert_df['ROC_AUC']
count_recall = count_df['ROC_AUC']
gemini_recall = gemini_df['ROC_AUC']
tfidf_recall = tfidf_df['ROC_AUC']
word2vec_recall = word2vec_df['ROC_AUC']
...
cbar = plt.colorbar(im)
cbar.set_label('Score')
ax.set_title("Model ROC-AUC Performance Comparison")
fig.tight_layout()
